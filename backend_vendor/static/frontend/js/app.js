...
class SearchEngine {
    ...
    buildIndex(products) {
        ...
        products.forEach(product => {
            ...
            // Index category (handle both string and object format)
            if (product.category) {
                const categoryName = typeof product.category === 'string'
                    ? product.category
                    : (product.category.name || '');
                if (categoryName) {
                    this.trie.insert(categoryName, product);
                    this.addToIndex(categoryName.toLowerCase(), product);
                }
            }
...
class ECommerceApp {
    ...
    // Get store name - returns vendor name if in vendor store mode, otherwise default
    getStoreName() {
        if (window.VENDOR_CONFIG && window.VENDOR_CONFIG.name) {
            return window.VENDOR_CONFIG.name;
        }
        return 'OVN Store';
    }

    // Get store contact info - returns vendor info if available, otherwise defaults
    getStorePhone() {
        if (window.VENDOR_CONFIG && window.VENDOR_CONFIG.phone) {
            return window.VENDOR_CONFIG.phone;
        }
        return '9824236055';
    }

    getStoreEmail() {
        if (window.VENDOR_CONFIG && window.VENDOR_CONFIG.email) {
            return window.VENDOR_CONFIG.email;
        }
        return 'support@ovnstore.com';
    }

    getStoreAddress() {
        if (window.VENDOR_CONFIG && window.VENDOR_CONFIG.address) {
            return window.VENDOR_CONFIG.address;
        }
        return 'Basundhara, Kathmandu';
    }
        ...
        try {
            ...
            if (this.products && this.products.length > 0) {
                try {
                    ...
                    console.warn('⚠️ Failed to build search index:', searchError);
                }
            }
            // Note: renderApp is called by init() after loadInitialData completes
            console.log('📦 Initial data loaded, ready for rendering');
        }
    }
    ...
    setupEventListeners() {
        ...
        document.addEventListener('click', (e) => {
            ...
            // Category icon click
            if (e.target.matches('.category-icon-item') || e.target.closest('.category-icon-item')) {
                e.preventDefault();
                const categoryItem = e.target.matches('.category-icon-item') ? e.target : e.target.closest('.category-icon-item');
                const categorySlug = categoryItem.dataset.category;
                if (categorySlug) {
                    this.navigate(`/products?category=${encodeURIComponent(categorySlug)}`);
                }
            }
        });
        ...
        document.addEventListener('submit', (e) => {
            if (e.target.matches('.login-form') || e.target.matches('.modern-login-form')) {
                e.preventDefault();
                // In demo mode, show message instead of actual login/register
                if (window.IS_DEMO_MODE) {
                    const isRegister = e.target.matches('.register-form-clean');
                    const action = isRegister ? 'Registration' : 'Login';
                    this.showNotification(`${action} is disabled in demo mode. Start your free trial to enable all features!`, 'info');
                    return;
                }
    ...
    navigate(route) {
        // In demo mode, block navigation to login/register pages
        if (window.IS_DEMO_MODE && (route === '/login' || route === '/register')) {
            const action = route === '/login' ? 'Login' : 'Registration';
            this.showNotification(`${action} is disabled in demo mode. Start your free trial to enable all features!`, 'info');
            return;
        }
    ...
    renderApp() {
        ...
        // In demo mode, strip the demo base path for internal routing
        if (window.IS_DEMO_MODE && window.DEMO_BASE_PATH) {
            if (currentPath.startsWith(window.DEMO_BASE_PATH)) {
                currentPath = currentPath.replace(window.DEMO_BASE_PATH, '') || '/';
            }
        }

        // In vendor store mode, strip the vendor base path for internal routing
        if (window.IS_VENDOR_STORE && window.VENDOR_BASE_PATH) {
            if (currentPath.startsWith(window.VENDOR_BASE_PATH)) {
                currentPath = currentPath.replace(window.VENDOR_BASE_PATH, '') || '/';
            }
        }

        // In demo mode, redirect away from login/register pages
        if (window.IS_DEMO_MODE && (currentPath === '/login' || currentPath === '/register')) {
            const action = currentPath === '/login' ? 'Login' : 'Registration';
            this.showNotification(`${action} is disabled in demo mode. Start your free trial to enable all features!`, 'info');
            history.replaceState({}, '', '/');
            currentPath = '/'; // Update to render home page
        }
        ...
        switch (true) {
            case currentPath === '/products':
                ...
                break;
            case currentPath === '/flash-sale':
                content = this.renderFlashSalePage();
                break;
                    ...
                    <div class="header-left">
                        <div class="logo">
                            <a href="/" data-route="/" style="text-decoration: none; color: inherit; display: flex; align-items: center; gap: 12px;">
                                ${window.VENDOR_CONFIG && window.VENDOR_CONFIG.logo
                                    ? `<img src="${window.VENDOR_CONFIG.logo}" alt="${this.getStoreName()}" style="width: 52px; height: 52px; border-radius: 12px; object-fit: cover; box-shadow: 0 4px 12px rgba(102, 126, 234, 0.3);">`
                                    : `<div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); width: 52px; height: 52px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-weight: 900; font-size: 26px; color: white; box-shadow: 0 4px 12px rgba(102, 126, 234, 0.3);">
                                    ${this.getStoreName().charAt(0).toUpperCase()}
                                </div>`}
                                <span class="logo-text" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; font-weight: 800; font-size: 24px;">${this.getStoreName()}</span>
                            </a>
                        </div>
                    </div>
                            ...
                            ${this.currentUser ? `
                                <div class="user-menu">
                                    ...
                                    <div class="user-actions-menu">
                                        ...
                                        <a href="/account/" style="display: flex; align-items: center; gap: 8px; padding: 12px 20px; color: #374151; text-decoration: none; border-radius: 8px; margin-bottom: 8px; background: #f3f4f6; font-weight: 500;">
                                            <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/>
                                            </svg>
                                            My Account
                                        </a>
                                        ${this.currentUser.has_vendor ? `
                                            <a href="/vendor/" style="display: flex; align-items: center; gap: 8px; padding: 12px 20px; color: white; text-decoration: none; border-radius: 8px; margin-bottom: 8px; background: linear-gradient(135deg, #10b981 0%, #34d399 100%); font-weight: 600;">
                                                <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
                                        ...
                                        ` : (!this.currentUser.is_staff && !this.currentUser.is_superuser) ? `
                                            <a href="/vendor/become-vendor/" style="display: flex; align-items: center; gap: 8px; padding: 12px 20px; color: white; text-decoration: none; border-radius: 8px; margin-bottom: 8px; background: linear-gradient(135deg, #10b981 0%, #34d399 100%); font-weight: 600;">
                                                <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24">
                                                    <path d="M20 4H4v2h16V4zm1 10v-2l-1-5H4l-1 5v2h1v6h10v-6h4v6h2v-6h1zm-9 4H6v-4h6v4z"/>
                                                </svg>
                                                Become a Vendor
    ...
    renderHomePage() {
        // Get section visibility and order settings from vendor config
        const sections = window.VENDOR_CONFIG?.sections || {};
        const sectionOrder = window.VENDOR_CONFIG?.sectionOrder || ['hero', 'categories', 'featured', 'new-arrivals', 'deals', 'newsletter'];

        // Map section IDs to render functions and visibility
        const sectionMap = {
            'hero': { show: sections.show_hero !== false, render: () => this.renderHeroSection() },
            'categories': { show: sections.show_categories !== false, render: () => this.renderFeaturedCategories() },
            'featured': { show: sections.show_featured !== false, render: () => this.renderFeaturedProducts() },
            'new-arrivals': { show: sections.show_new_arrivals !== false, render: () => this.renderNewArrivalsSection() },
            'deals': { show: sections.show_flash_sale !== false, render: () => this.renderFlashSaleSection() },
            'newsletter': { show: sections.show_newsletter !== false, render: () => '' } // Newsletter not implemented yet
        };

        // Render sections in the specified order
        const renderedSections = sectionOrder
            .filter(sectionId => sectionMap[sectionId]?.show)
            .map(sectionId => sectionMap[sectionId]?.render() || '')
            .join('');
    ...
    renderNewArrivalsSection() {
        // Filter products created in last 30 days or just take the newest ones
                    ...
                    <div class="products-grid">
                        ${newProducts.map(product => {
                            ...
                            if (comparePrice && comparePrice !== price) {
                                const higher = Math.max(price, comparePrice);
                                const lower = Math.min(price, comparePrice);
                                discount = Math.round(((higher - lower) / higher) * 100);
                            }

                            return `
                                <div class="product-card" onclick="window.app.navigate('/product/${product.id}')">
                                    ...
                                    <div class="product-info">
                                        ...
                                        <div class="product-price">
                                            <span class="current-price">Rs. ${formatPrice(Math.min(price, comparePrice || price))}</span>
                                            ${comparePrice && comparePrice !== price ? `<span class="original-price">Rs. ${formatPrice(Math.max(price, comparePrice))}</span>` : ''}
                                        </div>
                                    </div>
                                </div>
                            `;
    ...
    renderFlashSaleSection() {
        ...
        const formatPrice = (num) => num.toLocaleString('en-IN');
        const maxDisplay = 5;
        const displayProducts = this.flashSaleProducts.slice(0, maxDisplay);
        const hasMore = this.flashSaleProducts.length > maxDisplay;
            ...
            <section class="flash-sale-section">
                <div class="container">
                    <div class="flash-sale-header">
                        ...
                        ${hasMore ? `
                        <a href="#" class="flash-sale-view-all" onclick="event.preventDefault(); window.app.navigate('/flash-sale');">
    ...
    renderFlashSalePage() {
        ...
        return `
            <div class="flash-sale-page">
                <div class="container">
                    <div class="page-header flash-sale-page-header">
                        <div class="flash-sale-title-large">
                            <i class="fas fa-bolt"></i>
                            <h1>Flash Sale</h1>
                            <span class="flash-badge">Limited Time</span>
                        </div>
                        <p class="flash-sale-subtitle">${this.flashSaleProducts.length} products on sale</p>
                    </div>
                    <div class="flash-sale-grid">
                        ${this.flashSaleProducts.map(product => {
                            ...
                            return `
                                <div class="flash-sale-page-card" onclick="window.app.navigate('/product/${product.id}')">
                                    <div class="flash-sale-page-image">
                                        <img src="${product.image}" alt="${product.name}" loading="lazy">
                                        ${discount > 0 ? `<div class="flash-discount-badge">-${discount}%</div>` : ''}
                                    </div>
                                    <div class="flash-sale-page-info">
                                        <h3 class="flash-page-product-name">${product.name}</h3>
                                        <div class="flash-page-rating">
                                            ${this.renderStars(product.rating || 0)}
                                            <span>(${product.reviews || 0})</span>
                                        </div>
                                        <div class="flash-page-price">
                                            <span class="flash-current-price">Rs. ${formatPrice(sellingPrice)}</span>
                                            ${originalPrice ? `<span class="flash-original-price">Rs. ${formatPrice(originalPrice)}</span>` : ''}
                                        </div>
                                        <button class="flash-add-to-cart-btn" onclick="event.stopPropagation(); window.app.addToCart(${product.id});">
                        ...
                        ${this.categories.map(category => `
                            <div class="category-icon-item" data-category="${category.slug}">
                                <div class="category-icon-circle">
                                    <i class="fas ${category.icon || 'fa-shopping-basket'}"></i>
                                </div>
    ...
    renderFooter() {
        return `
            <footer style="background: none; padding: 20px 0; text-align: center;">
                <p style="color: #000; margin: 0; font-size: 0.9rem;">&copy; 2024 ${this.getStoreName()}. All rights reserved.</p>
            </footer>
        `;
    }
                ...
                <div class="auth-container-clean">
                    ...
                    <div class="auth-header-clean">
                        <div class="brand-logo-clean">
                            ${window.VENDOR_CONFIG && window.VENDOR_CONFIG.logo
                                ? `<img src="${window.VENDOR_CONFIG.logo}" alt="${this.getStoreName()}" style="width: 48px; height: 48px; border-radius: 12px; object-fit: cover;">`
                                : `<span class="logo-icon-clean">🛍️</span>`}
                            <span class="logo-text-clean">${this.getStoreName()}</span>
                        </div>
    ...
    renderProductsPage() {
        ...
        if (categorySlug) {
            ...
            displayProducts = this.products.filter(p =>
                p.category && (p.category.slug === categorySlug || p.category_slug === categorySlug)
            );
            pageTitle = categoryName;
            pageSubtitle = `${displayProducts.length} product${displayProducts.length !== 1 ? 's' : ''} in this category`;
            isFiltered = true;
            filterType = 'category';
        }
        // Search filter
        ...
        // No results message
        let noResultsHtml = '';
        if (isFiltered && displayProducts.length === 0) {
                            ...
                            <a href="/products?search=${encodeURIComponent(category.name)}" data-route="/products?search=${encodeURIComponent(category.name)}" class="category-card-link">
                                <div class="category-card-full">
                                    ...
                                    <div class="category-card-content">
                                        ...
                                        <p class="category-card-count">${this.products.filter(p => (typeof p.category === 'string' ? p.category : p.category?.name) === category.name).length} products</p>
                                    </div>
                                </div>
                            </a>
                        `).join('')}
            ...
            <div class="about-page">
                <div class="about-hero">
                    <div class="about-hero-content">
                        <h1 class="about-title">About ${this.getStoreName()}</h1>
                ...
                <div class="container">
                    <div class="about-content">
                        <section class="about-section">
                            ...
                            <h2>Our Story</h2>
                            <p>${this.getStoreName()} was founded with a simple mission: to bring quality products to customers across Nepal with exceptional service and competitive prices. What started as a small venture has grown into a trusted e-commerce platform serving thousands of happy customers nationwide.</p>
                        </section>
                    ...
                    <div class="contact-content">
                        <div class="contact-info-section">
                            <div class="contact-card">
                                ...
                                <h3>Our Location</h3>
                                <p class="contact-detail">${this.getStoreAddress()}</p>
                            ...
                            <div class="contact-card">
                                ...
                                <h3>Phone Number</h3>
                                <p class="contact-detail">${this.getStorePhone()}</p>
                            ...
                            <div class="contact-card">
                                ...
                                <h3>Email Address</h3>
                                <p class="contact-detail">${this.getStoreEmail()}</p>
                        ...
                        <div class="product-detail-grid">
                            ...
                            <div class="product-info-section">
                                <div class="product-badges">
                                    <span class="category-badge">${typeof product.category === 'string' ? product.category : (product.category?.name || 'Uncategorized')}</span>
    ...
    loginWithGoogle() {
        // In demo mode, show message instead of actual login
        if (window.IS_DEMO_MODE) {
            this.showNotification('Google Sign-In is disabled in demo mode. Start your free trial to enable all features!', 'info');
            return;
        }
            ...
            <div class="order-confirmation-page">
                <div class="container">
                    <div class="confirmation-content">
                        ...
                        <p class="confirmation-subtitle">Thank you for shopping with ${this.getStoreName()}</p>
                    ...
                    ${review.admin_response ? `
                        <div class="admin-response">
                            <div class="response-header">
                                ...
                                <strong>Response from ${this.getStoreName()}</strong>
                            </div>
                            <p>${review.admin_response}</p>
                        </div>
                    ` : ''}
                    ...
                    <div class="search-result-item" data-product-id="${product.id}" data-index="${index}">
                        ...
                        <div class="search-result-info">
                            ...
                            <div class="search-result-meta">
                                <span class="search-result-category">${this.escapeHtml(typeof product.category === 'string' ? product.category : (product.category?.name || 'Uncategorized'))}</span>
    ...
    async handleLogin(form) {
        // In demo mode, show message instead of actual login
        if (window.IS_DEMO_MODE) {
            this.showNotification('Login is disabled in demo mode. Start your free trial to enable all features!', 'info');
            return;
        }
